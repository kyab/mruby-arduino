...
This mrbgem is currently intended to use with Arduino compatible board "chipKIT Max32" from Digilent Inc, "Arduino Due",  or "Intel Galileo" but should work with any Arduino compatible environment. (Notice: Your board should have enough RAM, say around 90kb, to run mruby).
...
At first you should make cross-build setting for chipKIT Max32.  Copy contents of  /path/to/[mruby](https://github.com/mruby/mruby)/examples/targets/chipKitMax32.rb to /path/to/mruby/build_config.rb (or any file specified by MRUBY_CONFIG environment variable) .
...
Notice you don't have to download/clone mruby-arduino from github by yourself. mruby's make system automatically clone mrbgems from this repositry.
...
##Arduino Due
###1. Build mruby with mruby-arduino
At first you should make cross-build setting for Arduino Due, Copy contents of your /path/to/[mruby](https://github.com/mruby/mruby)/examples/targets/ArduinoDue.rb to /path/to/mruby/build_config.rb (or any file specified by MRUBY_CONFIG environment variable) .
...
##Intel Galileo
###1. Build mruby with mruby-arduino
At first you should make cross-build setting for Arduino IDE for Intel Galileo, Copy contents of [sample build_config.rb for Intel Galileo](https://gist.github.com/kyab/9187447) to /path/to/mruby/build_config.rb (or any file specified by MRUBY_CONFIG environment variable) .
...
On Intel Galileo, Arduino API is implemented on linux and sketch runs as one linux process, so has some limitations/lack of API(this is not mruby-arduino issue). I strongly recommend you to consult  [Intel Galileo Release Notes](https://communities.intel.com/docs/DOC-21837) .
